...
// write a general function that will retrieve all <input>, <textarea>, and <select> values from any <div> block
...
// searches for a hub by an attribute. This is usually either "id" or "hubid" in the cm_Globals.hubs object
function findHub(hubid, attr = "hubid") {
    hubid = hubid.toString();
    const thehub = cm_Globals.hubs.find( hub => hubid === hub[attr].toString() );
    return thehub;
}
    ...
    $.post(cm_Globals.returnURL, {api: "getdevices", userid: userid, hpcode: hpcode},
        function(presult, pstatus) {
            if (pstatus==="success" && typeof presult === "object") {
                ...
                console.error("error - could not load devices; needed for links and lists to work and to use the customizer. status: ", pstatus, " presult: ", presult);
            }
        }, "json"
    );
}
        ...
        $.post(cm_Globals.returnURL, 
            ...
            function (presult, pstatus) {
                if (pstatus==="success" && typeof presult === "object") {
                    ...
                    cm_Globals.hubs = {};
                    console.error("error - failure reading hubs from database for user = " + userid, " status: ", pstatus, " presult: ", presult);
                }
                setupButtons();
            }, "json"
        );
...
$(document).ready(function() {
    ...
    try {
        const mainhost = $("input[name='returnURL']").val();
        const mainport = $("input[name='webDisplayPort']").val();
        const pythonhost = $("input[name='pythonURL']").val();
        const pyport = $("input[name='pythonPort']").val(); 
        cm_Globals.protocol = window.location.protocol;
        cm_Globals.port = mainport;
        cm_Globals.pythonPort = pyport;
        cm_Globals.returnURL =  mainhost;
        cm_Globals.pythonURL = pythonhost;
        // console.log("mainhost: ", mainhost, " mainport: ", mainport, " pyhost: ", pythonhost, " pyport: ", pyport);
                        ...
                        $.post(cm_Globals.returnURL, 
                            ...
                            function (presult, pstatus) {
                                if ( pstatus==="success" ) {
                                    ...
                                    console.warn("Something went wrong, no customizations were updated. pstatus: ", pstatus, " presult: ", presult);
                                }
                            }
                        );                    
                    }
                    ...
                    $.post(cm_Globals.returnURL, 
                        ...
                        function (presult, pstatus) {
                            if ( pstatus==="success" ) {
                                ...
                                console.warn("Something went wrong, so no customizations were updated. pstatus: ", pstatus, " presult: ", presult);
                            }
                        }
                    );                    
                }
    ...
    if ( $("button.formbutton") ) {
        $("button.formbutton").on('click', function(evt) {
            ...
            if ( $(this).hasClass("confirm") || buttonid.startsWith("c__") ) {
                ...
                createModal("modalexec","Perform " + textname + " operation... Are you sure?", "body", true, pos, function(ui, content) {
                    ...
                    if ( clk==="okay" ) {
                        if ( pagename==="options" ) {
                            // handle options page specific button actions here
                            handleOptionsPageButton(buttonid, evt);
                        } else {
                            execButton(buttonid);
                        }
                    }
...
function setupWebsocket(userid, webSocketUrl) {
    ...
    wsSocket.onmessage = function (evt) {
        ...
        try {
            ...
            // handle case where server is requesting that we move a tile
            // it is done here so that all active clients receive the update and move the tile for consistency
            // otherwise only the client that moved the tile would update and all others would be out of sync
            // until they refreshed or moved that tile themselves
            } else if (bid==="setposition" ) {
                var thingid = pvalue.thingid;
                var thing = $("#t-"+thingid);
                if ( thing ) {
                    console.log("Received request to relocate tile for thingid: ", thingid, " with position value: ", pvalue);
            ...
            // this functionality was removed from HousePanel.groovy but I keep it here to request that users update their file
                ...
                if ( cm_Globals.dbgflags && cm_Globals.dbgflags["debug12"] ) {
                    console.log("webSocket update for bid: ", bid, " type: ", thetype, " pvalue: ", pvalue);
                }

                // if this is a motion, contact, or switch message then call the python server to update the AI context for automations
                // if ( thetype==="motion" || thetype==="contact" || thetype==="switch" || thetype==="presence" || thetype==="lock" ) {
                //     $.post(cm_Globals.pythonURL, 
                //         {api: "ai-context", userid: userid, type: thetype, value: pvalue, hpcode: cm_Globals.options.hpcode},
                //         function (presult, pstatus) {
                //             if ( pstatus==="success" ) {
                //                 if ( cm_Globals.dbgflags && cm_Globals.dbgflags["debug18"] ) {
                //                     console.log("Context update sent to server for type: ", thetype, " with value: ", pvalue, " result: ", presult);
                //                 }
                //             } else {
                //                 console.warn("Something went wrong sending context update to server for type: ", thetype, " with value: ", pvalue, " pstatus: ", pstatus, " presult: ", presult);
                //             }
                //         }
                //     );                    
                // }
...
async function createModal(modalid, modalcontent, modaltag, addok,  pos, responsefunction, loadfunction) {
    // var modalid = "modalid";

    // make a promise version for selectors
    // const once = (sel, evt, childSel) => new Promise(resolve =>
    // childSel ? $(sel).one(evt, childSel, resolve) : $(sel).one(evt, resolve)
    // );
    ...
    // return Promise.any([
    //         once("#"+modalid, "tap", ".dialogbtn"),
    //         once("#"+modalid + " .menuitem", "tap"),
    //         once("body", "singletap")
    // ])
    // .then(async response => {
    //     var result = true;
    //     var evt = response.originalEvent || response;
    //     var evt_origtype = response.handleObj.origType;
    //     var evt_type = response.handleObj.type;
    //     var evt_selector = response.handleObj.selector;
    //     console.log("Event handlers set up for modal: ", modalid, " evt_type: ", evt_type, " origType: ", evt_origtype, " selector: ", evt_selector, " evt.target: ", evt.target );
    //     closeModal(modalid);

    //     // return and do nothing if we used keyboard and didn't hit return
    //     // if ( evt_type==="keydown" ) {
    //     //     if ( evt.which == 13 ) {
    //     //         result = await responsefunction(evt.target, modaldata);
    //     //     } else {
    //     //         result = false;
    //     //     }
    //     // }

    //     if ( evt_type==="tap"&& evt_selector===".dialogbtn" && responsefunction ) {
    //         result = await responsefunction(evt.target, modaldata);
    //         // return result;
    //     }

    //     if ( modalid==="menubox" && evt_type==="tap" && $(evt).hasClass("menuitem") && responsefunction ) {
    //         result = await responsefunction(evt.target, modaldata);
    //         // return result;
    //     }

    //     if ( evt_type==="singletap" && responsefunction ) {
    //         result = await responsefunction(evt.target, modaldata);
    //         // return result;
    //     } 
    ...
    // })
    // .catch(error => {
    //     console.error("Error setting up event handlers for modal: ", modalid, " error: ", error);
    //     closeModal(modalid);
    //     return false;
    // });

    // the old way didn't allow me to return a value from the response function
    // so I rewrote it above using Promise.any to wait for any of the possible events and then process them in a single handler
    if ( addok ) {
        $("#"+modalid).on("keydown", async function(e) {
            if ( e.which===13 ) {
                if ( responsefunction ) {
...
function handleUploadSubmit(formEl, userid, pname, iCategory, hpcode, successFunction, errorFunction, cancelFunction) {
    var imageInput = $(formEl).find('#imageInput')[0];
    if ( imageInput && imageInput.files.length > 0 ) {
        var thefile = imageInput.files[0];
        // console.log("Image selected to upload: ", thefile);
    } else {
        cancelFunction("No file selected");
        closeModal("modalupl");
        return;
    }

    // get the form and submit via ajax
    var myform = new FormData(formEl);
    // console.log("userid: ", userid, " pname: ", pname, " iCategory: ", iCategory, " hpcode: ", hpcode);
    $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            console.error("Upload failed with status: ", textStatus, " error: ", errorThrown);
...
function uploadImage(userid, iCategory, startFunction, successFunction, errorFunction, cancelFunction, pos=null) {
    if (pos === null) {
        pos = {top: 100, left: 150, width: 300};
    }
    ...
    var htmlcontent =  "<div class=\"uploaddialog\"><h3>Upload File</h3><br>";
    htmlcontent += `<strong>Select file to upload to ${catText}:</strong><br><br>
        ...
        <div>
        <input type="file" id="imageInput" accept="image/*,video/*" name="uploaded_file" />
        </div>
    ...
    createModal("modalupl", htmlcontent, "body", false, pos,
        ...
        function() {
            ...
            $("#uplcancel").on("click", function(evt) {
                cancelFunction();
                closeModal("modalupl");
            });
            ...
            $("form.upload").on("submit", function(e) {
                e.preventDefault();
                handleUploadSubmit(this, userid, pname, iCategory, hpcode, successFunction, errorFunction, cancelFunction);
            });
        }
    );
}
   ...
   $("div.overlay.color >div.color").each( function() {
        ...
        $(this).minicolors({
            ...
            hide: function() {
                ...
                const linkinfo = handleLinks(subid, uid, bid, thingid);
                ...
                if ( cm_Globals.dbgflags["debug14"] ) {
                ...
                $.post(cm_Globals.returnURL, 
                    ...
                    subid: realsubid, attr: hexval, hubindex: linkhub, tileid: tileid, command: command, linkval: linkval, hpcode: cm_Globals.options.hpcode},
                    function(presult, pstatus) {
                        if ( pstatus==="success" && typeof presult === "object" ) {
                            console.log("Action result: ", presult);
                        } else {
                            console.warn("Color action post failed for user: ", userid, " deviceid: ", linkbid, " status: ", pstatus, " presult: ", presult);
                        }
                    }, "json"
                );
            }
...
function hpsliders(evt, ui) {
    ...
    const linkedtile = cm_Globals.devices[linkuid];
    var linkhub = linkedtile["hubid"];   // this is the hubindex of the linked tile which is what we need to send to the server to identify the hub for this action
                                         // it is unfortunate that the field name for the hubindex in the devices table is "hubid" which is confusing but changing it would be a pain at this point
    var linkbid = linkedtile["deviceid"];
    var linktype = linkedtile["devicetype"];
    var hint = linkedtile["hint"];
    ...
    $.post(cm_Globals.returnURL, 
        ...
        function(presult, pstatus) {
            if ( pstatus==="success" && typeof presult === "object" ) {
                ...
                console.warn("Slider action post failed for user: ", userid, " deviceid: ", linkbid, " status: ", pstatus, " presult: ", presult);
            }
        }, "json"
    );
}
    ...
    function thingDraggable(thing, snap, container) {
        ...
        thing.draggable({
            ...
            stop: function(evt, ui) {
                ...
                if ( cm_Globals.dbgflags["debug1"] ) {
                    console.log("Dragged thing: ", thingtype, " tileid: ", tileid, " left: ", startPos.left, " top: ", startPos.top, " bid: ", bid, "uid: ", uid );
                }
                ...
                $.post(cm_Globals.returnURL, 
                    ...
                    function (presult, pstatus) {
                        if (pstatus==="success" && typeof presult==="object" ) {
                            // don't need to do this here because the server updates all clients
                            // relocateTile(thing, presult);
                            if ( cm_Globals.dbgflags["debug1"] || DEBUGtmp ) {
                                console.log("setposition success: ", presult);
                            }
                        } else {
                            console.warn("setposition post failed for user: ", cm_Globals.options.userid, " deviceid: ", bid, " status: ", pstatus, " presult: ", presult);
                        }
                    }, "json"
                );
            }
                                ...
                                $.post(cm_Globals.returnURL, 
                                    ...
                                    function (presult, pstatus) {
                                        if (pstatus==="success" && typeof presult==="string" && !presult.startsWith("error") ) {
                                            if ( cm_Globals.dbgflags["debug1"] ) {
                                                console.log("Added new tile to room: ", panel, " type: ", thingtype, " id: " , bid);
                                            }
...
function rehomeTiles() {
    ...
    $("div.thing[panel="+panel+"]").each( function() {
        ...
        $.post(cm_Globals.returnURL, 
            ...
            function (presult, pstatus) {
                if ( pstatus!=="success" ) {
                    console.error("Something went wrong when rehoming tiles. status: ", pstatus, " result: ", presult);
                }
            }
        );
...
function dynoPost(ajaxcall, body, callback) {
    ...
    // if body is not given or is not an object then make a basic one
    ...
    if ( callback && typeof callback==="function" ) {
        ...
        $.post(cm_Globals.returnURL, body,
            function (presult, pstatus) {
                console.log("dynoPost result: ", presult, " status: ", pstatus);
            }
        );
    }
}
...
function handleOptionsPageButton(buttonid, evt) {
    // handle options page specific button actions here

    // do nothing but reload the main page
    if ( buttonid==="optionsdone" ) {
        evt.stopPropagation();
        window.location.href = cm_Globals.returnURL;
        return;
    }

    var oobj = divToObject(buttonid+"div");
    const xpos = 100; // parseInt($(evt.target).position().left);
    const ypos = parseInt($(evt.target).position().top) - 300;
    
    // get the hubless hub id
    let hubzero = findHub("-1", "hubid");
    let thingtype = buttonid.replace("options", "");

    $.post(cm_Globals.returnURL, 
        {api: buttonid, userid: cm_Globals.options.userid, type: thingtype, value: oobj, hubindex: hubzero.id, hpcode: cm_Globals.options.hpcode}, 
        function(presult, pstatus) {
            if ( pstatus==="success" && typeof presult === "object" ) {
                var pstyle = `position: absolute; border: 6px black solid; background-color: blue; color: white; font-weight: bold; font-size: 18px; left: ${xpos}px; top: ${ypos}px; width: 600px; height: auto; padding: 30px;`;
                var pos = {style: pstyle};

                // for image and video options, refresh the preview
                // we assume the info needed to display the preview is returned in the appropriate presult.pvalue fields
                if ( buttonid === "imageoptions" || buttonid === "videooptions" || buttonid === "frameoptions" ) {
                    popupMessage(thingtype + " options saved successfully", 3000, pos);
                    // createModal("popupinfo", thingtype + " options saved successfully.", "body", "Okay", pos);
                    $("#"+thingtype+"preview").html(decodeURI(presult[thingtype]));
                    $("#"+thingtype+"nameid").val(presult.name);
                    $("#"+thingtype+"widthid").val(presult.width);
                    $("#"+thingtype+"heightid").val(presult.height);
                    $("#"+thingtype+"urlid").val(decodeURI(presult.url));
                    if ( thingtype === "frame" && presult.data ) {
                        $("#framecontent").val(decodeURI(presult.data));
                        // $("#framecontent").focus();
                    // } else {
                ...
                var pos = {style: pstyle};
                createModal("popupinfo", "Function " + buttonid + " failed to save properly. " + presult, "body", "Okay", pos);
            }
        }
    );
}
...
function execButton(buttonid) {
    if ( buttonid==="devSave") {
        ...
        dynoPost("savedevices", oobj, function(presult, pstatus) {
            if ( pstatus!=="success" ) {
                alert("Save Devices failed, please try again. If this problem continues please contact support.");
                window.location.href = cm_Globals.returnURL;
            } else {
                window.location.href = cm_Globals.returnURL;
            }
    ...
    } else if ( buttonid==="devCancel" ) {
        // do nothing but reload the main page
        window.location.href = cm_Globals.returnURL;
        return;

    } else if ( buttonid==="devReset") {
        // reset the forms on the deviced edit page to their starting values
        $("#deveditpage")[0].reset();
        ...
        dynoPost("dologin", genobj, function(presult, pstatus) {
            if ( pstatus === "success" && presult && typeof presult === "object" ) {
                ...
                createModal("loginfo","User Email: " + presult["email"] + "<br>Username: " + presult["uname"] + "<br><br>Page loading in 3 seconds... ",
                            "body", false, pos);
                setTimeout(function() {
                    closeModal("loginfo");
                    window.location.href = cm_Globals.returnURL;
        ...
        popupMessage("Snap mode " + (cm_Globals.snap ? "enabled" : "disabled"), 2000, {position: "absolute", top: 50, left: 50, backgroundColor: "blue", color: "white", fontSize: "18px", padding: "10px", borderRadius: "5px"});
    ...
    // these are commands that open a separate web page and are only supported in operate mode, otherwise they will be ignored
...
function updateFilters() {
    ...
    dynoPost("filteroptions", fobj, function(presult, pstatus) {
        if ( pstatus!=="success" ) {
            alert("Updating hub filters failed, please try again. If this problem continues please contact support.");
            window.location.href = cm_Globals.returnURL;
        } else {
            window.location.href = cm_Globals.returnURL;
        }
...
function setupButtons() {
    ...
    if ( pagename==="main" ) {
        ...
        $("div.filteroption").on("keydown", function(evt) {
            if ( evt.which === 27 ) {
                window.location.href = cm_Globals.returnURL;
            }
        });

        // handlers for the selection options
        // get the existing content from server by attempting to read the file
        $("#whichframe").on("change", specialHandler);
        $("#whichframe").trigger("change");

        // handler for images
        $("#whichimage").on("change", specialHandler);
        $("#whichimage").trigger("change");

        $("#whichvideo").on("change", specialHandler);
        $("#whichvideo").trigger("change");
        
        $("#newUsername").focus();

        function specialHandler(evt) {
            const imagenum = $(this).val();
            const thingtype = $(this).attr("id").replace("which", "");
            const userid = cm_Globals.options.userid;
            $.post(cm_Globals.returnURL, {api: "getspecialcontent", value: imagenum, type: thingtype, userid: userid, hpcode: cm_Globals.options.hpcode},
                function(presult, pstatus) {
                if ( pstatus==="success" && typeof presult==="object") {
                    $("#"+thingtype+"preview").html(decodeURI(presult[thingtype]));
                    $("#"+thingtype+"nameid").val(presult.name);
                    $("#"+thingtype+"widthid").val(presult.width);
                    $("#"+thingtype+"heightid").val(presult.height);
                    $("#"+thingtype+"id").val(presult.id);
                    if ( thingtype === "frame" && presult.data ) {
                        $("#framecontent").val(decodeURI(presult.data));
                        $("#"+thingtype+"urlid").val("user"+userid+"/Frame"+imagenum+".html");
                    } else {
                        $("#"+thingtype+"urlid").val(decodeURI(presult.url));
                    }
                }
            }).catch(function(err) {
                console.warn("Error loading special content for: ", imagenum, " error: ", err);
                $("#"+thingtype+"urlid").val("");
                $("#"+thingtype+"preview").html("");
                if ( thingtype === "frame" ) {
                    $("#framecontent").val("");
                }
            });
        ...
        function getImageNativeSize(thingtype, url, onSuccess, onError) {
            var img = new Image();
            img.onload = function() {
        ...
        function getMediaNativeSize(thingtype, url, onSuccess, onError) {
            
            if (thingtype === "video" ) {
                var video = document.createElement('video');
                video.onloadedmetadata = function() {
                    onSuccess({ width: video.videoWidth, height: video.videoHeight });
                    video.src = ""; // stop loading the video to free up resources
                    video = null;
                };
                ...
                img.onload = function() {
                    onSuccess({ width: img.naturalWidth, height: img.naturalHeight });
                    img.src = ""; // stop loading the image to free up resources
                    img = null;
                };
                img.onerror = function(err) {
                    if (onError) onError(err);
        ...
        // pick image and pick video buttons
        // borrows code from imgupload handler to open the url in a new tab for previewing
        $("div.options button.pickbutton").on("click", function(evt) {
            const thingtype = $(this).attr("name");
            var mouseY = evt.originalEvent.pageY;
            let xpos = 100;
            let ypos = parseInt(mouseY) - 150;
            ypos = ypos < 100 ? 100 : ypos;
            const pstyle = `position: absolute; border: 4px black solid; background-color: white; color: black; font-size: 14px; left: ${xpos}px; top: ${ypos}px; width: 600px; height: auto; padding: 10px;`;
            uploadImage(cm_Globals.options.userid, "User_Media", null,
            function(response) {
                if ( cm_Globals.dbgflags["debug1"] ) {
                    console.log("response from upload: ", response);
                }
                let urlfield = "#"+thingtype+"urlid";
                let widthfield = "#"+thingtype+"widthid";
                let heightfield = "#"+thingtype+"heightid";
                let saveid = "#"+thingtype+"options";
                if ( response && response.path ) {
                    $(urlfield).val(response.path);
                    getMediaNativeSize(thingtype, response.path, function(size) {
                        let w = size.width;
                        let h = size.height;
                        // set a max dimension to prevent huge images from breaking the layout
                        const maxDim = 640; 
                    ...
                    console.warn("No url returned from upload for " + thingtype);
                    $(urlfield).val("");
                }
            },
            function(err) {
                console.warn("Error picking " + thingtype + ": ", err);
            }, function() {
                console.warn("No file selected for " + thingtype);
            }, {style: pstyle});
            evt.stopPropagation();
        });
...
function setupAuthHub(hubId, userid) {
    ...
    // handle new hubs - user sets type and the Hubitat groovy app ills out the rest of the fields
    if ( hubId==="new" ) {
        $("#hubdiv").show();
...
function addEditLink() {
    ...
    $("div.rstlink").on("click",function(evt) {
        ...
        var startPos = {top: 0, left: 0, "z-index": 1, position: "relative"};
        if (str_type==="bulb") {
            startPos["z-index"] = getMaxZindex(panel) + 1;
        }
...
function updateTile(aid, presult) {
    ...
    for (var key in presult) {
        ...
        if ( (typeof target === "object") && (typeof value !== "object") && !(typeof value==="string" && value.startsWith("{")) ) {
            ...
            // if ( (isNaN(pn) || pn >= 0) && key.startsWith("_") && !key.startsWith("_set") && !key.startsWith("_number_") ) {
    ...
    if ( cm_Globals.options && cm_Globals.options.userid && cm_Globals.options.rules && cm_Globals.options.hpcode ) {
        ...
        clocktiles.each( function() {
            ...
            if ( updateDatabase ) {
                ...
                if ( cm_Globals.dbgflags["debug1"] ) {
                    console.log("Updating clock for uid = " + uid, " devicee: ", cm_Globals.devices[uid]);
                }
...
function setupPage() {
    ...
    $("div.thing div.overlay > div").on("singletap", function(evt) {
        ...
        // clicking on slider handle does nothing
        ...
        // this is all handled by the ClickWithValue function that has been generalized to handle lists and dicts
        ...
        // but we skip _set commands since those are handled below with pn value for the number of parameters
        ...
        //     processClickWithValue(that, thingname, ro, subid, thetype, "", numParams);
            ...
            // get the value for setLevel and setPosition commands
            if ( subid.startsWith("_set") && isNumeric(thevalue) ) {
                thevalue = parseInt(thevalue);
                processClickWithValue(that, thingname, ro, subid, thetype, thevalue, pn);
            } else {
...
function processClickWithValue(that, thingname, ro, subid, thetype, thevalues, numParams=0) {
    ...
    if ( cm_Globals.dbgflags["debug1"] ) {
        console.log("processClickWithValue: subid=", subid, " thevalues=", thevalues, " numParams=", numParams);
    }
    if ( typeof thevalues === "object" ) {
        var prefixes = Object.keys(thevalues);
        var prevalues = Object.values(thevalues);
    ...
    for (var i = 0; i < numParams; i++) {
        if ( prefixes && prevalues ) {
            prefix = prefixes[i].trim();
            thevalue = prevalues[i];

            // handle case where there is a single parameter but the prefix is blank - use subid as prefix since that is more descriptive for user
            // note that the caller can't set prefix to blank for more than one parameter
            if ( prefix==="" ) {
                prefix = subid;
            }
        } else if ( numParams===1 ) {
        ...
        var id = "newsubid" + idstr;
        if ( typeof thevalue === "object" && Array.isArray(thevalue) ) {
            htmlcontent += `<div class='ddlDialog'><label for="${id}">${prefix}: </label>`;
            htmlcontent += `<select id="${id}" name="picklist${idstr}" class="picklist">`;
...
function handleLinks(subid, uid, bid, thingid) {
    ...
    if ( cm_Globals.options.rules && cm_Globals.options.rules[usrbid] ) {
        ...
        linkrules.forEach( rule => {
            ...
            if ( rule[0] === "LINK" && rule[2] === linkvalupdown  ) {
                ...
                linkval = rule[1];
                command = "RULE";
            } else if ( (rule[0] === "GET" || rule[0]==="POST" || rule[0]==="PUT" || rule[0]==="URL" || rule[0]==="AI") && rule[2] === subid ) {
                linkval = rule[1];
                command = rule[0];
    ...
    // if user provided "ask" as the custom linkval then we request their input here using createModal
    // const askcommands = ["GET", "POST", "PUT", "URL", "AI"];
    // if ( askcommands.includes(command) && ( linkval === "ask" || linkval === "prompt" ) ) {
    //     const userid = cm_Globals.options.userid;
    //     var tpos = $(targetid).offset();
    //     var ttop = (tpos.top > 125) ? tpos.top - 120 : 5;
    //     var pos = {top: ttop, left: 100};
    //     var htmlcontent = "<h3>Enter value for command: " + command + "</h3>";
    //     htmlcontent += "<div class='ddlDialog'><label for='userinput'>Value:</label>";
    //     htmlcontent += "<input class='ddlDialog' id='userinput' type='text' size='60' value='' />";
    //     htmlcontent += "</div>";

    //     var results = [command, null, realsubid, linkuid];
    //     createModal("modalexec", htmlcontent, "body", true, pos, 
    //     function(ui) {
    //         var clk = $(ui).attr("name");
    //         if ( clk==="okay" ) {
...
function processClick(that, thingname, ro, thevalue, theattr = true, subid  = null) {
    ...
    if ( cm_Globals.dbgflags["debug1"] ) {
        console.log("processClick: thingname: " + thingname + " subid: " + subid + " thevalue: " + thevalue + " ro: " + ro + " theattr: " + theattr);
    }
    const userid = cm_Globals.options.userid;
    const pname = cm_Globals.options.pname;
    const tile = '#t-'+taid;
    const pyEndpoint = $(tile).attr("py");
    ...
    // determine which server to call for this action - if this is a python endpoint then use the python URL otherwise use the standard return URL
    const actionPostUrl = (pyEndpoint || command==="PYTHON") ? cm_Globals.pythonURL : cm_Globals.returnURL;
    ...
    // removed frame, image, and video from passive list so that clicking on them will update their content instead of doing nothing
    // fixed subid here to be realsubid - which allows me to check for exact matches instead of startsWith for linked fields
    let ispassive = ro;
    if ( command==="RULE" || command==="LIST" || command==="GET" || command==="POST" || command==="PUT"  || command==="AI" || command===ro ) {
        ispassive = ro;
    } else {
        ispassive = (ispassive || command==="TEXT" || realsubid==="thingname" || realsubid==="custom" || realsubid==="temperature" || realsubid==="feelsLike" || realsubid==="battery" || 
            realsubid==="presence" || realsubid==="motion" || realsubid==="contact" || realsubid==="status_" || realsubid==="status" || realsubid==="deviceType" || realsubid==="localExec" ||
            realsubid==="time" || realsubid==="date" || realsubid==="tzone" || realsubid==="weekday" || realsubid==="name" || realsubid==="skin" || realsubid==="thermostatOperatingState" ||
            realsubid==="pushed" || realsubid==="held" || realsubid==="doubleTapped" || realsubid==="released" || realsubid==="numberOfButtons" || realsubid==="humidity" ||
            realsubid==="blank" || realsubid.startsWith("event_") || realsubid==="illuminance" || realsubid==="ecoMode" ||
            realsubid==="colorMode" || realsubid==="colorName" || realsubid.startsWith("label") || realsubid.startsWith("text")
        );
    }

    if ( ispassive || realsubid==="image" || realsubid==="frame" || realsubid==="video" ) {
        console.log("Tile clicked:", thingname, ", subid=", subid, ", realsubid=", realsubid, ", command=", command);
    }
    ...
    if ( command==="" && ( (thetype==="momentary" && realsubid==="momentary") || 
        ...
        $.post(actionPostUrl, 
            ...
            function(presult, pstatus) {
                if (pstatus==="success" && typeof presult === "object" ) {
                    console.log(thetype, "Action result: ", presult);
                    if (thetype==="piston") {
                    ...
                    console.error("Momentary/piston action failed: pstatus=", pstatus, ", presult=", presult);
                }
            }, 
        "json");
    ...
    // process user provided allon or alloff fields that turn all lights and switches on or off
        ...
        $('div[panel="' + panel + '"] div.overlay.switch div').each(function() {
            ...
            if ( thevalue ) {
                ...
                function(presult, pstatus) {
                    if (pstatus==="success" && typeof presult === "object" ) {
                        console.log(subid, "Action result: ", presult);
                    } else {
                        console.error("action failed for deviceid= ", bid,  " as part of an ", subid, " call, pstatus=", pstatus, ", presult=", presult);
                    };
                });
            }
        });
        ...
        // this isn't needed for HE but I put this here for ISY
        ...
        // however, I still inverted the HE values to support future update
        ...
        // provide a simple one line log of the action
        const logval = linkval ? linkval : thevalue;
        const commandval = command ? command : "Action";
        if ( cm_Globals.dbgflags["debug1"] || realsubid==="frame" || realsubid==="image" || realsubid==="video" ) {
            console.log(`${commandval}: User[${userid}], ${thingname}, subid[${realsubid}] = ${logval}, deviceid[${linkbid}], thingid[${thingid}], hubindex[${linkhub}]`);
        }
        ...
        $.post(actionPostUrl, 
            ...
            function (presult, pstatus) {
                if ( pstatus==="success" && typeof presult === "object" ) {
                    ...
                    if ( command==="LIST" ) {
                        ...
                        $("#resetList").on("tap", function(evt) {
                            $.post(actionPostUrl, 
                                ...
                                function (presult, pstatus) {
                                    if ( pstatus==="success" && typeof presult==="string" && !presult.startsWith("error") ) {
                                        popupMessage(presult, 4000);
                                    } else {
                                        console.error("List reset failed: pstatus=", pstatus, ", presult=", presult);
                                    }
                                }, "json"
                            );
                            $("#resetList").off("tap");
                    ...
                    const errmsg = pstatus==="success" ? "" : "Request failed with status: " + pstatus;
                    console.warn(errmsg, "Action result: ", presult);
                }
            }, "json"
        );
    } 
