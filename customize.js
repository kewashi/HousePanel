...
function customizeTile(userid, tileid, uid, bid, thingid, str_type, hubnum) {  
    ...
    var customname;
    try {
        customname = $("#a-"+thingid+"-name").text();
        if ( !customname ) {
            customname = $("#s-"+thingid).text();
        }
    } catch(e) {
        customname = "";
    }
        ...
        createModal("modalcustom", dh, "body", "Done", pos, 
            ...
            function() {
                ...
                if ( !cm_Globals.devices ) {
                    alert("error - you have no devices to use in the Tile Customoizer ...");
                    closeModal("modalcustom");
                    return;
                } else {
                    try {
...
function getExistingFields(tileid, curval) {
    ...
    for ( var i in sortdevices ) {
        ...
        var uid = sensor["uid"].toString();
        var hubname = sensor["hubname"];
        ...
        if ( itemnum > 1 ) {
            ...
            $.post(cm_Globals.returnURL, 
                {api: "updcustom", userid: cm_Globals.options.userid, id: bid, hpcode: cm_Globals.options.hpcode,
                 rules: rules, tileid: tileid, uid: uid, subid: subid, value: null, attr: null},
                function (presult, pstatus) {
                    if (pstatus==="success") {
